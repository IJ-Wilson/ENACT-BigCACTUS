...
% 
% %  Edited by Laura Flight for CACTUS case study 18Aug22
...
basic.sigma = 10894.66;    % sample standard deviation
basic.mu0 = 3190.78;    % prior: mean for unknown mean
basic.t0 = 7;
%sigma0 = basic.sigma/sqrt(basic.t0);   % prior: std dev for unknown mean

% Patient delay and pool
basic.numpairs = 95;    % 124;
basic.TMax = 95;    % total number of patients in the population for testing
basic.c = 4706;    % variable cost per patient of doing trial

delayinyears = 12/12;    % one month is 1/12.
accrualperiod=(638.751)/365.25;    % accural period 
...
basic.tau = ceil(patientsperyear * delayinyears);
...
basic.PPatients = 215378;    % total number of patients to be treated following adoption
basic.ICost = 0;    % fixed cost of investment
...
basic.mumax = 20000 ;    % maximum posterior mean per patient; pick big enough to be above upper stopping boundary
basic.mumin = -20000;    % minimum posterior mean per patient; pick small enought to be below lower stopping boundary
...
advanced.titlestring='CSLT vs UC';    % for use in figures / display
advanced.filestring='BC';     % for use in saving files - should be a string with only characters, no blanks etc.
advanced.dirstring='BC';     % for use in saving files - should be a string with only characters, no blanks etc.
